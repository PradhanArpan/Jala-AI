...
// Layer 1 of the WIM framework: the built and natural water system
// itself. Coverage is community-maintained and uneven, so a tank
// absent here is unmapped, not necessarily absent on the ground.
// ============================================================
...
(
  ...
  node["man_made"~"^(water_well|water_tower|water_works|reservoir_covered)$"](around:${r},${lat},${lon});
...
//
// `layer` is the GIBS identifier. If a layer renders blank at every
// date, the identifier is wrong or retired — check against NASA
// Worldview rather than assuming the map is broken.
//
// `static: true` marks products with no daily time dimension, so
// the date stepper is not applied to them.
// ============================================================
const SAT_LAYERS = [
  {
    id: 'truecolor',
    ...
    opacity: 1,
    legend:
      'VIIRS corrected reflectance — what the satellite sees on the day. ' +
      'Cloud, sediment plumes, snow and flooding are visible directly.',
  },
  {
    id: 'precip',
    label: 'Precipitation',
    ...
    opacity: 0.75,
    legend:
      'GPM IMERG precipitation rate — half-hourly global rainfall from the ' +
      'satellite constellation. Blue is light, red is intense.',
  },
  {
    id: 'soil',
    label: 'Soil moisture',
    group: 'Water',
    layer: 'SMAP_L3_Passive_Day_Soil_Moisture',
    opacity: 0.8,
    legend:
      'SMAP passive soil moisture — water held in the top few centimetres ' +
      'of soil. The bridge between rainfall and what a crop actually gets.',
  },
  {
    id: 'snow',
    label: 'Snow cover',
    ...
    opacity: 0.8,
    legend:
      'MODIS snow cover — seasonal water storage. For Indian rivers this is ' +
      'the Himalayan contribution that sustains dry-season flow.',
  },
  {
    id: 'sst',
    ...
    opacity: 0.8,
    legend:
      'Sea surface temperature — a driver of monsoon behaviour. Arabian Sea ' +
      'and Bay of Bengal warmth shapes how much moisture reaches the ' +
      'subcontinent.',
  },
  {
    id: 'lst',
    ...
    opacity: 0.75,
    legend:
      'MODIS daytime land surface temperature — the skin temperature of the ' +
      'ground, a strong proxy for drought and heat stress.',
  },
  {
    id: 'ndvi',
    label: 'Vegetation',
    ...
    opacity: 0.8,
    legend:
      'MODIS NDVI, 8-day composite — vegetation vigour. Green is dense ' +
      'growth; pale is sparse or stressed cover.',
  },
  {
    id: 'fires',
    label: 'Fires',
    ...
    opacity: 0.9,
    legend:
      'MODIS thermal anomalies — active fire detections. Clusters in the ' +
      'dry season often mark crop residue burning rather than wildfire.',
  },
  {
    id: 'aerosol',
    label: 'Aerosol',
    ...
    opacity: 0.75,
    legend:
      'Aerosol optical depth — how much dust, smoke and pollution is in the ' +
      'air column. High values over the Indo-Gangetic plain are seasonal.',
  },
  {
    id: 'nightlights',
    label: 'Night lights',
    ...
    opacity: 0.9,
    legend:
      'VIIRS day/night band — settlement and activity at night. A rough ' +
      'proxy for where water demand concentrates.',
  },
]
      ...
      g.items.forEach((el) => {
        if (el.type === 'node' && el.lat) {
          L.circleMarker([el.lat, el.lon], {
            ...
            .bindTooltip(
              (el.tags && (el.tags.name || el.tags.man_made || el.tags.amenity)) ||
                g.label
            )
            .addTo(layer)
          ...
          shape.bindTooltip((el.tags && el.tags.name) || g.label)
          shape.addTo(layer)
        }
      })
    })
...
function App() {
  ...
  function toggleSat(id) {
    setSatLayers((prev) => {
      ...
      else next[id] = SAT_LAYERS.find((x) => x.id === id)?.opacity ?? 0.8
      return next
    })
  }
      ...
      <div hidden={view !== 'observatory'} className="workspace">
        ...
        <div className="workbench">
          <section className="readout" aria-label="Live readout">
            {activeSats.length > 0 && (
              <div>
                ...
                <p className="group-note">
                  Imagery from NASA GIBS for {prettyDate(satDate)}. Polar-orbiting
                  satellites build each day from successive passes, so a region
                  can be incomplete; step back a day if you hit a gap.
                </p>
              </div>
            )}
            ...
            {place && showOsm && osm && (
              <div>
                <h2 className="group-head">Physical systems · OpenStreetMap</h2>
                {osm.failed ? (
                  ...
                  <>
                    ...
                    <p className="group-note">
                      ...
                      uneven: a tank missing here is unmapped, not necessarily
                      absent on the ground. Named features show their name on
                      hover.
                    </p>
                  </>
                )}
